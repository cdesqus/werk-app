...
import {
    ...
    FileText, Zap, Sparkles, LogOut, Bell, Moon, Sun, Search
} from 'lucide-react';
...
const AdminLayout = () => {
    ...
    const NavItem = ({ to, icon: Icon, label }) => (
        <NavLink to={to} end={to === '/admin'} className={({ isActive }) => clsx(
    ...
    return (
        <div className="premium-bg flex font-sans selection:bg-lime-400 selection:text-black">
            {/* Sidebar */}
            <aside className="w-[280px] fixed h-screen bg-black/20 backdrop-blur-3xl border-r border-white/5 flex flex-col z-50">
                {/* Header */}
                <div className="p-6 border-b border-white/5">
                    <div className="font-black tracking-tighter text-white flex items-start select-none">
            ...
            <main className="flex-1 ml-[280px] min-h-screen">
                {/* Top Bar */}
                <header className="h-16 border-b border-white/5 bg-black/20 backdrop-blur-md sticky top-0 z-40 flex items-center justify-between px-8">
